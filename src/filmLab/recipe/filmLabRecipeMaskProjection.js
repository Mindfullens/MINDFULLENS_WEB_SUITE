...
function serializeMaskSlot(mask, index) {
  ...
  return {
    ...
    tone: m.tone && typeof m.tone === 'object' ? m.tone : {},
...
function serializeLiveSlotFromAdjustments(adj) {
  return {
    ...
    source: null,
    tone: {
      contrast: Number(adj.brushMaskContrast ?? 0),
      highlights: Number(adj.brushMaskHighlights ?? 0),
      shadows: Number(adj.brushMaskShadows ?? 0),
      whites: Number(adj.brushMaskWhites ?? 0),
      blacks: Number(adj.brushMaskBlacks ?? 0),
      temp: Number(adj.brushMaskTemp ?? 0),
      tint: Number(adj.brushMaskTint ?? 0),
      saturation: Number(adj.brushMaskSaturation ?? 0),
    },
    brush: {
      radius: Number(adj.brushMaskRadius ?? 32),
