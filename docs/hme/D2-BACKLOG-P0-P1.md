...
| **D2-P1-02** | **Referencyjny zestaw vs RAW gate:** polityka zamknięta w [`RELEASE-GATE-POLICY.md`](RELEASE-GATE-POLICY.md) — warstwy A (`npm run ci`), B (`raw:reference:gate`), C (`reference-set-v1` + skrypt sync). | `npm run raw:reference:gate` vs `npm run test:reference-set-v1-assets` — kiedy które: patrz dokument. | Łukasz | |
